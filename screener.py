...
_SCALPING_CFG_TEMPLATE: dict[str, Any] = {
    "enabled": True,
    # 오프닝레인지 브레이크아웃(ORB) -- 09:00-09:05 레인지를 거래량+VWAP+
    ...
    #
    # timeframe 3Min -> 1Min (2026-08-27, 사용자 요청): 돌파는 그 3분봉이
    # 마감돼야 신호가 나오므로, 3분봉에서는 돌파 후 최대 3분까지 늦게 반응할
    # 수 있었다. 1분봉으로 낮추면 그 지연이 최대 1분으로 줄고, 오프닝
    # 레인지 자체도 더 촘촘하게(분 단위로) 잡힌다. 봉 마감 후에만 판단하는
    # 무-룩어헤드 원칙은 그대로 -- 아직 안 끝난 봉을 보지는 않는다.
    #
    ...
    # 안전장치다.
    #
    # range_minutes 15 -> 5, entry_window 시작 09:15 -> 09:05 (2026-08-27,
    # 사용자 요청 - 테스트): 장 시작 직후 반등폭이 커서 그 구간을 더 일찍
    # 잡아보자는 시도. 레인지가 5봉(1분봉)만으로 잡히는 만큼 기준선 자체가
    # 더 노이즈에 취약해진다는 트레이드오프가 있다 -- 사용자에게 이미 설명함.
    "strategy": "orb",
    "timeframe": "1Min",
    "min_qty": 1,
    ...
    "params": {
        ...
        # 11시~15시 사이 신규 진입은 손절/확정 폭을 대칭 1.5%/1.5%로 고정
        # (2026-08-28, 사용자 요청: "너무 거래가 없네 -- 익절1.5% 손절1.5%로
        # 가자 11시부터3시까지"). 회전을 빠르게 해서 체결 빈도를 늘리려는
        # 목적 -- 진입 시점 기준으로 한 번 정해지면 그 포지션이 살아있는
        # 동안 계속 유지된다.
        "midday_stop_pct": 0.015,
        "midday_lock_pct": 0.015,
        "midday_window_start": "11:00",
        "midday_window_end": "15:00",
        "arm_pct": 0.012,
        "lock_pct": 0.025,
        # peak_trail_pct 0.3%->0.5%, big_win_pct 4%->3% (2026-08-28, 사용자
        # 요청): 익절 후 종목이 눌림 없이 계속 오르는데도 좁은 0.3% 트레일에
        # 미리 털리고, 이미 판 자리라 재진입 쿨다운에 막혀 못 쫓아가는 문제 --
        # 트레일을 넓히고 넓은 트레일이 붙는 문턱도 낮춘다 (재진입 쿨다운
        # 자체는 익절 후 재진입에 한해 건너뛰도록 main.py에서 별도로 바꿈).
        # big_win_trail_pct(고점 대비 트레일) -> big_win_floor_pct(진입가
        # 기준 고정 바닥) (2026-08-31, 사용자 요청: "3%이상 오르면 2%로
        # 아래 떨어질때까지 기다리다 팔기로") -- 고점을 따라가며 트레일하지
        # 않고, 3% 넘긴 뒤로는 그 이후 고점이 얼마나 더 올라도 진입가+2%
        # 밑으로 내려올 때까지 버틴다.
        "peak_trail_pct": 0.005,
        "big_win_pct": 0.03,
        "big_win_floor_pct": 0.02,
        "max_cost_share": 0.35,
    },
}
...
_PULLBACK_CFG_TEMPLATE: dict[str, Any] = {
    ...
    #
    # timeframe 3Min -> 1Min (2026-08-27, 사용자 요청): 눌림목 재돌파도 봉
    # 마감 기준이라, 3분봉에서는 반등 시작부터 최대 3분 늦게 잡을 수 있었다.
    # 1분봉으로 최대 지연을 1분으로 줄인다. swing_lookback=7/pullback_bars=2는
    # 봉 개수 기준이라 그대로 두면 "7분/2분" 창으로 자동으로 더 짧아진다 --
    # 노이즈에 좀 더 민감해지는 트레이드오프는 있다.
