...
class PullbackBounce(Strategy):
    ...
    def __init__(
        ...
        #: fib_max 50%->61.8% (2026-09-18, 사용자 요청 -- "너무 매매가 없다":
        #: 2026-09-18 아침 로그 분석 결과 이 필터가 EMA 추세 필터 다음으로
        #: 진입을 많이 막고 있었고(53~82% 되돌림 구간에서 다수 거부), 기술적
        #: 분석에서 표준으로 쓰는 "황금 되돌림 구간"은 애초에 38.2%~61.8%다
        #: -- 임의로 완화하는 게 아니라 원래 정석 구간으로 넓히는 것. EMA
        #: 추세 필터는 건드리지 않음 -- 승률 33% 문제(가짜 신호)의 핵심
        #: 원인이었던 confirm_bars와 달리, 이 필터를 넓힌다고 그 문제가
        #: 다시 커지지는 않는다는 판단.
