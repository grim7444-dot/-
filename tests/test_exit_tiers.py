...
# ---------------------------------------------------------------------------
# market_down / bad_market_lock_pct: on a day the stock's own index
...
def test_market_down_locks_in_a_profit_the_normal_lock_pct_would_have_missed(make_strategy):
    ...
    price = floor - 1.0
    signal = strategy.evaluate(_window(price), _position(entry, peak), market_down=True)
    assert signal.action is Action.EXIT
    assert "확정 익절" in signal.reason
